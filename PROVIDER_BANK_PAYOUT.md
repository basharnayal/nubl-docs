...
We intentionally do not create a wallet OUT transaction at payout-request generation time because, at that moment, no real bank transfer has happened yet. The generated payout is only a settlement request for admin review, not an actual cash movement. In our accounting model, wallet balances must only change when a real FundTransaction is created through the normal observer flow, and this happens only after the admin uploads a receipt and confirms the external bank transfer. This keeps ewallets.balance aligned with real financial events and avoids “phantom” balance changes.

If we created the OUT immediately at generation time, we would be reducing the provider’s wallet balance before the transfer was actually executed. That would create accounting problems in cases where the payout is later rejected, cancelled, delayed, or corrected. We would then need reversal entries or manual compensation logic, which adds unnecessary complexity and increases the risk of ledger inconsistencies. The implemented design avoids that by separating request creation from money movement.

Why we use provider_payout_items instead of simply paying from provider ewallet.balance

We do not use the raw wallet balance alone because ewallets.balance is only the final aggregate number. It does not tell us which specific credits are:

eligible provider earnings,
already reserved by another payout request,
already settled in a previous payout,
or unrelated wallet entries.

For weekly settlement, we need to settle only the provider’s real earned credits, which come from provider wallet IN rows with source PAYOUT created by the existing redemption flow. The eligibility logic explicitly filters those rows and excludes any rows already linked to an active or completed payout in reserving statuses. This is why provider_payout_items exists: it gives us a precise, auditable link between one payout request and the exact earning ledger lines that make up that payout amount.

What provider_payout_items gives us

Using provider_payout_items instead of raw balance gives the system four important guarantees:

No double settlement
The same earning line cannot be accidentally included in two active payouts. The settlement flow excludes rows already linked to payouts in reserving statuses.
Full auditability
We can always trace a payout request back to the exact fund_transactions that produced its amount. This is much stronger than saying “the wallet had enough balance.”
Safe reject/cancel behavior
If a payout is rejected or cancelled, no OUT is created, and those linked earning lines become eligible again for a later weekly run. That would be much harder to manage correctly if we had already deducted from the balance.
Correct financial meaning
The wallet balance is the overall current balance, but settlement must operate on eligible earning lines, not on the balance as a blind total. This keeps the payout process aligned with the real business meaning of provider earnings.

Reference summary

The implemented model is:

Provider earns internal wallet credit at redemption, via provider wallet IN rows with source PAYOUT.
Weekly generation creates a payout request plus provider_payout_items, but does not move wallet balance yet.
Admin review + receipt upload + confirmation creates exactly one real provider wallet OUT with source PROVIDER_BANK_PAYOUT. That is the moment the ledger and wallet balance actually change.

One-line team answer

We did not create FundTransaction OUT at generation time because generation is only a payout request, not a real transfer; and we used provider_payout_items instead of raw ewallet.balance because settlement must be based on exact eligible earning ledger lines, not on the wallet total alone.